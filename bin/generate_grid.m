function [lon_sub,lat_sub,depth_sub] = generate_grid(ref_dir,bathy_source,...
    coord,dx,dy,limit,cut_off,dry)
...
%|                                                                        |
%| [lon,lat,depth] = generate_grid(ref_dir,bathy_source,coord,dx,dy,...   |
%|                                                  limit,cut_off,dry)    |
%|                                                                        |
%|  INPUT                                                                 |
...
%|                    being used. User needs to make sure that the        |
%|                    bathymetry data files corresponding to the options  |
%|                    are available in ref_dir                            |
%|                    Options are --                                      |
...
%|    limit         : Value ranging between 0 and 1 indicating what       |
%|                    fraction of a grid cell needs to be covered by wet  |
%|                    cells (from the base grid) for the cell to be       | 
%|                    marked wet.                                         |
%|    cut_off       : Cut_off depth to distinguish between dry and wet    |
%|                    cells, All depths below the cut_off depth are       |
%|                    marked as wet cells                                 | 
%|    dry           : Depth value assigned to the dry cells               |
%|                                                                        |
%|  OUTPUT                                                                |
%|    lon           : An array of length Nx consisting of the longitude   |
%|                    (x) coordinates of the grid                         |
%|    lat           : An array of length Ny consisting of the lattitude   |
%|                    (y) coordinates of the grid                         |
%|    depth         : A 2D array of dimensions (Nx,Ny) consisting of the  |
%|                    grid depths                                         |
%|                                                                        |
%|   NOTES                                                                |
...
%|       base grids, make sure thatfeatures are defined in the base       |
...
%|                                                                        |
%|   BUG FIXES                                                            |
%|      06/25/2012 : Fixed erroneous generation of NaNs in grids that     |
%|                   wrap around the globe                                |
%|      10/23/2012 : Removed coord choice (-180 to 180 or 0 360). Now     |
%|                   grids are defined only from 0 to 360 (to avoid       |
%|                   spurious boundaries near the edges)                  |
% -------------------------------------------------------------------------
...
%@@@ Code assumes that the longitude data in source file is stored in 
%@@@ -180 to 180 range
...
if (ndx <= 1 && ndy <= 1)
    ...
    fprintf(1,'Target grid too fine, Interpolating from base ... \n');
    den = dx_base*dy_base;
    ...
    for i = 1:Nx
        ...
        dx1 = min(abs(lon_sub(i)-lon_base(lon_prev)),abs(lon_sub(i)-...
            lon_base(lon_prev)-360*sign(lon_sub(i)-lon_base(lon_prev))));
        dx2 = dx_base - dx1;
        ...
        for j = 1:Ny
            ...
            depth_sub(j,i) = (a11*dy2*dx2 + a12*dy2*dx1 + a21*dy1*dx2 + ...
                a22*dx1*dy1)/den;
    ...
    for i = 1:Nx
        ...
        for j = 1:Ny
            ...
            %@@@ Average the depth over all the wet cells in source that 
            %@@@ lie within the cell. Cell is marked dry if the proportion 
            %@@@ of wet cells in source is less than specified limit.

            if (lon_start_pos < lon_end_pos)               
                ...
                if (~isempty(loc1) || ~isempty(loc2))
                    ...
                    if (Ntt/Nt > limit)
                        depth_sub(j,i) = mean([depth_tmp1(loc1) ; ...
                            depth_tmp2(loc2)]);
                    else
                        depth_sub(j,i) = dry;
                    end;
